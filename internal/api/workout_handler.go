...
func (wh *WorkoutHandler) HandleCreateWorkoutByID(w http.ResponseWriter, r *http.Request) {
...
func (wh *WorkoutHandler) HandleUpdateWorkoutByID(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		wh.logger.Printf("ERROR: decodingUpdateWorkoutRequest: %v", err)
	...
	currentUser := middleware.GetUser(r)
	if currentUser == nil || currentUser == store.AnonymousUser {
		utils.WriteJSON(w, http.StatusBadRequest, utils.Envelope{"error": "must be logged in to update"})
		return
	}
